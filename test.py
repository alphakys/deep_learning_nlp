...
[sentence for sentences in train_data]
[tokenizer.texts_to_matrix(sentence) for sentence in train_data]
[sentence for sentence in train_data]
tokenizer.texts_to_matrix('I enjoy coffee')
train_sequences = tokenizer.texts_to_sequences(train_data)
to_categorical()
from keras.utils import to_categorical
l = [1,3,4,4,3,3]
to_categorical(l)
to_categorical(l, num_classes=3)
to_categorical(l, num_classes=4)
to_categorical(l, num_classes=5)
np.array(l)
l = np.array(l)
l.shape
l.shape[-1]
...
np.append(l, 'rr')
np.max(l)
ts = tokenizer.texts_to_sequences(train_data)
ts
del ts
to_categorical(train_sequences)
from keras.utils import Sequence
to_categorical(train_sequences[0])
...
list(word_to_index.values())[-1]
max_num_classes = list(word_to_index.values())[-1]
to_categorical(train_sequences, num_classes=max_num_classes)
to_categorical(train_sequences[0], num_classes=max_num_classes)
word_to_index
len(to_categorical(train_sequences[0], num_classes=max_num_classes)[0])
to_categorical(train_sequences[0], num_classes=max_num_classes+1)
to_categorical(train_sequences[1], num_classes=max_num_classes+1)
to_categorical(train_sequences[2], num_classes=max_num_classes+1)
train_sequences[1]
train_sequences[2]
train_sequences[3]
to_categorical(train_sequences[3], num_classes=max_num_classes+1)
y = [to_categorical(seq, num_classes=max_num_classes+1) for seq in train_sequences]
tokenizer.texts_to_matrix(text)
tokenizer.texts_to_matrix(train_data)
y
del l
pad_sequences(train_sequences)
pad_sequences(train_sequences[0])
train_sequences[0]
train_sequences
train_data
...
tk.word_index
tk.texts_to_sequences(test_data)
